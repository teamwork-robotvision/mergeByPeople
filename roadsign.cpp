        ...
        if (contours.at(index).size() >= 4 /*轮廓角点初步筛选*/
            ...
            if (cnt.angle > angleLimit[0]&& cnt.angle <=angleLimit[1]) {//矩阵外界角度筛选
                ...
                if(middle >= wid_hei[0]&&middle <= wid_hei[1]&&cnt.size.area() > 100 && cnt.size.area() < sizeOfArea) {//矩阵长宽比和矩阵大小筛选
                    ...
                    int wid=ROI.cols>p->getUI()->roadSign0->width()? p->getUI()->roadSign0->width():ROI.cols;
                    int hei=ROI.rows>p->getUI()->roadSign0->height()?p->getUI()->roadSign0->height():ROI.rows;
                    if(count==0){
                        p->getUI()->roadSign0->setPixmap( QPixmap::fromImage(image.scaled(wid,hei)));
                        //p->ui->outputImage->resize(100,100);
                        p->getUI()->roadSign0->show();
                    }else if(count==1){
                        p->getUI()->roadSign1->setPixmap( QPixmap::fromImage(image.scaled(wid,hei)));
                        //p->ui->outputImage->resize(100,100);
                        p->getUI()->roadSign1->show();
                    }else{
                        p->getUI()->roadSign2->setPixmap( QPixmap::fromImage(image.scaled(wid,hei)));
                        //p->ui->outputImage->resize(100,100);
                        p->getUI()->roadSign2->show();
                    }
                    count++;
                    count%=3;


                    //                    int x=box[0].x,y=box[0].y;
                    //                    for(int i=1;i<4;i++){
                    //                        if(x<box[i].x)
                    //                            x=box[i].x;
                    //                        if(y<box[i].y)
                    //                            y=box[i].y;
                    //                    }
                    //                    Mat ROI=outputImage(cvRect(x,y,cnt.size.width,cnt.size.height));
